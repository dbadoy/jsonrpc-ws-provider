...
export interface DeferredPromiseInterface<T> extends Promise<T> {
...
export class DeferredPromise<T> implements Promise<T>, DeferredPromiseInterface<T> {
    ...
    public constructor(
        { timeout, eagerStart, timeoutMessage }: { timeout: number; eagerStart: boolean; timeoutMessage: string } = {
            timeout: 0,
            eagerStart: false,
    ...
    private _checkTimeout() {
        if (this._state === 'pending' && this._timeoutId) {
            this.reject(new Error(this._timeoutMessage));
        }
    }
