...
export class JsonRpcWebSocketProvider extends EventEmitter {
  ...
  protected _reconnect(): void {
    ...
    if (this._sentRequestsQueue.size > 0) {
      this._sentRequestsQueue.forEach(
        (request: SocketRequestItem<any>, key: JsonRpcId) => {
          request.deferredPromise.reject(
            new PendingRequestsOnReconnectingError(),
          );
    ...
    if (this._reconnectAttempts < this._reconnectOptions.maxAttempts) {
      ...
      this.emit(
        'error',
        new MaxAttemptsReachedOnReconnectingError(
  ...
  private _addSocketListeners(): void {
    ...
    this._socketConnection?.addEventListener('message', (rawData) =>
      this._onMessageHandler(rawData),
    );
    this._socketConnection?.addEventListener('close', (e) =>
      this._onCloseHandler(e),
    );
    this._socketConnection?.addEventListener('error', (e) =>
